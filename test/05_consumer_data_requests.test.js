...
contract("Consumer - data request tests", (accounts) => {
  ...
  const dataProvider2Pk = "0xadd53f9a7e588d003326d1cbf9e4a43c061aadd9bc938c843a79e7b4fd2ad743"
  ...
  describe("ideal scenario - enough tokens and allowance, dataProvider authorised", function () {
    ...
    describe("initialise requests", function () {
      ...
      it("dataConsumer (owner) can initialise a custom request - emits CustomDataRequested event", async function () {
        const reciept = await this.MockConsumerCustomRequestContract.customRequestData(
          dataProvider,
          endpoint,
          gasPrice,
          { from: dataConsumerOwner },
      ...
      it("request ID correctly generated", async function () {
        const expectedReqId = generateRequestId(
      ...
      it("dataConsumer (owner) can add rando as new data provider and initialise a request", async function () {
        ...
        const reciept = await this.MockConsumerContract.requestData(rando, endpoint, gasPrice, {
          from: dataConsumerOwner,
        })
        const reqId = await getReqIdFromReceipt(reciept)
      ...
      it("request will fail if dataProvider revoked - reverts with error", async function () {
        ...
        await expectRevert(
          this.MockConsumerContract.requestData(dataProvider, endpoint, gasPrice, {
            from: dataConsumerOwner,
          }),
          "ConsumerLib: _dataProvider is not authorised",
        )
  ...
  describe("advanced token and payment tests", function () {
    ...
    describe("single data provider", function () {
      it("data request success - ERC20 token balance for Consumer contract should be 800 after 2 requests", async function () {
        ...
        await this.MockTokenContract.transfer(this.MockConsumerContract.address, initialContract, {
          from: dataConsumerOwner,
        })
        // add Data provider
        await this.MockConsumerContract.addRemoveDataProvider(dataProvider, dpFee, false, {
          from: dataConsumerOwner,
        })
        // increase router allowance
        await this.MockConsumerContract.setRouterAllowance(new BN(999999 * 10 ** 9), true, {
        ...
        // request 1, pay 100
        const r1 = await this.MockConsumerContract.requestData(dataProvider, endpoint, gasPrice, {
          from: dataConsumerOwner,
        })
        // request 2, pay another 100
        const r2 = await this.MockConsumerContract.requestData(dataProvider, endpoint, gasPrice, {
          from: dataConsumerOwner,
        })
    ...
    describe("multiple data providers", function () {
      it("data request success - ERC20 token balance for Consumer contract should be 700 after 2 requests", async function () {
        ...
        // add Data provider1
        await this.MockConsumerContract.addRemoveDataProvider(dataProvider, dp1Fee, false, {
          from: dataConsumerOwner,
        })
        // add Data provider2
        await this.MockConsumerContract.addRemoveDataProvider(dataProvider2, dp2Fee, false, {
          from: dataConsumerOwner,
        })
        // increase router allowance
        await this.MockConsumerContract.setRouterAllowance(new BN(999999 * 10 ** 9), true, {
        ...
        const r2 = await this.MockConsumerContract.requestData(dataProvider2, endpoint, gasPrice, {
          from: dataConsumerOwner,
        })
    ...
    describe("multiple data consumers and providers", function () {
      it("data request success - ERC20 token balance for Consumer contracts should be correct after 4 requests", async function () {
        ...
        const MockConsumerContract1 = await MockConsumer.new(this.RouterContract.address, {
          from: dataConsumerOwner,
        })
        // add tokens to consumer contract
        await this.MockTokenContract.transfer(MockConsumerContract1.address, initialContract1, {
          from: dataConsumerOwner,
        })
        // add Data provider1
        await MockConsumerContract1.addRemoveDataProvider(dataProvider, c1p1Fee, false, {
          from: dataConsumerOwner,
        })
        // add Data provider2
        await MockConsumerContract1.addRemoveDataProvider(dataProvider2, c1p2Fee, false, {
          from: dataConsumerOwner,
        })
        // increase router allowance
        await MockConsumerContract1.setRouterAllowance(new BN(999999 * 10 ** 9), true, {
          from: dataConsumerOwner,
        })

        // dataConsumerOwner2 deploy Consumer contract
        const MockConsumerContract2 = await MockConsumer.new(this.RouterContract.address, {
          from: dataConsumerOwner2,
        })
        // add tokens to consumer contract
        await this.MockTokenContract.transfer(MockConsumerContract2.address, initialContract2, {
          from: dataConsumerOwner2,
        })
        // add Data provider1
        await MockConsumerContract2.addRemoveDataProvider(dataProvider, c2p1Fee, false, {
          from: dataConsumerOwner2,
        })
        // add Data provider2
        await MockConsumerContract2.addRemoveDataProvider(dataProvider2, c2p2Fee, false, {
          from: dataConsumerOwner2,
        })
        // increase router allowance
        await MockConsumerContract2.setRouterAllowance(new BN(999999 * 10 ** 9), true, {
          from: dataConsumerOwner2,
        })

        // request 1 - consumer 1 from provider 1, pay 100
        const r1 = await MockConsumerContract1.requestData(dataProvider, endpoint, gasPrice, {
          from: dataConsumerOwner,
        })

        // request 2 - consumer 1 from provider 2 pay 200
        const r2 = await MockConsumerContract1.requestData(dataProvider2, endpoint, gasPrice, {
          from: dataConsumerOwner,
        })

        // request 3 - consumer 2 from provider 1, pay 100
        const r3 = await MockConsumerContract2.requestData(dataProvider, endpoint, gasPrice, {
          from: dataConsumerOwner2,
        })

        // request 4 - consumer 2 from provider 2 pay 200
        const r4 = await MockConsumerContract2.requestData(dataProvider2, endpoint, gasPrice, {
          from: dataConsumerOwner2,
        })
