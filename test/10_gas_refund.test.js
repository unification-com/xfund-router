...
const MockBadConsumerBigReceive = artifacts.require("MockBadConsumerBigReceive") // Loads a compiled contract
const ConsumerLib = artifacts.require("ConsumerLib") // Loads a compiled contract

contract("Provider - gas refund tests", (accounts) => {
  ...
  beforeEach(async function () {
    ...
    this.MockBadConsumerBigReceiveContract = await MockBadConsumerBigReceive.new(
  ...
  describe("ideal scenario - enough tokens and allowance, dataProvider authorised, gas topped up", function () {
    ...
    beforeEach(async function () {
      ...
      await this.MockBadConsumerBigReceiveContract.setRouterAllowance(new BN(999999 * 10 ** 9), true, {
        from: dataConsumerOwner,
      })
      ...
      await this.MockBadConsumerBigReceiveContract.addRemoveDataProvider(dataProvider, fee, false, {
        from: dataConsumerOwner,
      })
      ...
      await this.MockTokenContract.transfer(
        this.MockBadConsumerBigReceiveContract.address,
      ...
      await this.MockBadConsumerBigReceiveContract.topUpGas(dataProvider, {
        from: dataConsumerOwner,
        value: topupValue,
      })
    ...
    describe("consumer pays gas", function () {
      ...
      it(`gas consumed diff <= acceptable threshold (<= ${MAX_ACCEPTABLE_GAS_DIFF_SET_NON_ZERO}) - second time`, async function () {
        ...
        const receipt1 = await this.MockConsumerContract.requestData(dataProvider, endpoint, gasPrice, {
          from: dataConsumerOwner,
        })
        ...
        const receipt2 = await this.MockConsumerContract.requestData(dataProvider, endpoint, gasPrice, {
          from: dataConsumerOwner,
        })
      ...
      if (!IS_COVERAGE) {
        it("provider end balance always >= start balance: 20 iterations", async function () {
          ...
          for (let i = 0; i < 20; i += 1) {
            ...
            const receipt = await this.MockConsumerContract.requestData(dataProvider, endpoint, randGas, {
              from: dataConsumerOwner,
            })
            const reqId = getReqIdFromReceipt(receipt)
        ...
        describe("bad consumer implementation", function () {
          it("fulfillRequest spend/refund diff always >= 0 when receiveData func is big: 50 iterations", async function () {
            for (let i = 0; i < 50; i += 1) {
              ...
              const receipt = await this.MockBadConsumerBigReceiveContract.requestData(
                dataProvider,
                endpoint,
                randGas,
                { from: dataConsumerOwner },
      ...
      describe("provider pays gas", function () {
        it("consumer balance on Router never deducted: 20 iterations", async function () {
          ...
          for (let i = 0; i < 20; i += 1) {
            ...
            const consumerRouterBalanceBefore = await this.RouterContract.getGasDepositsForConsumer(
              dataConsumerOwner,
            )
            const consumerProviderRouterBalanceBefore = await this.RouterContract.getGasDepositsForConsumerProviders(
              dataConsumerOwner,
              dataProvider,
            )
            const fulfullReceipt = await this.RouterContract.fulfillRequest(reqId, price, sig.signature, {
            ...
            const consumerRouterBalanceAfter = await this.RouterContract.getGasDepositsForConsumer(
              dataConsumerOwner,
            )
            const consumerProviderRouterBalanceAfter = await this.RouterContract.getGasDepositsForConsumerProviders(
              dataConsumerOwner,
              dataProvider,
            )

            expect(consumerRouterBalanceAfter).to.be.bignumber.equal(consumerRouterBalanceBefore)
            expect(consumerProviderRouterBalanceAfter).to.be.bignumber.equal(
              consumerProviderRouterBalanceBefore,
            )
          }
        })
